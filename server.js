                ...
                rows.each((rowIndex, row) => {
                    ...
                    if (cells.length >= 2) {
                        ...
                        if (matchName && matchData.length > 0) {
                            predictions.push({ match: matchName, aiData: matchData });
                        }
                    }
                });
            }
        });
        if (predictions.length > 0) { return { aiNames: aiNames, predictions: predictions }; }
        return null;
    } catch (error) { console.warn(`⚠️ 外部站点抓取失败: ${error.message}`); return null; }
}
...
function calculateKellyForMatches(matches, mode, budget = 100) {
    ...
    const getOddAndPred = (match, overrideMethod) => {
        ...
        if (method === '总进球数') {
            let odd = 1.9;
            if (odds.totalGoalsMap && odds.totalGoalsMap[prediction]) { odd = odds.totalGoalsMap[prediction]; }
            return { odd, pred: prediction };
        }
    ...
    if (matches.length === 4) {
        ...
        const pA = getConf(A.matchKey); const pB = getConf(B.matchKey); const pC = getConf(C.matchKey); const pD = getConf(D.matchKey);
        const oddA = getOddAndPred(A, mode).odd; const oddB = getOddAndPred(B, mode).odd; const oddC = getOddAndPred(C, mode).odd; const oddD = getOddAndPred(D, mode).odd;
        addCombo(`单关:${A.home}`, [A], pA, oddA); addCombo(`单关:${B.home}`, [B], pB, oddB); addCombo(`单关:${C.home}`, [C], pC, oddC); addCombo(`单关:${D.home}`, [D], pD, oddD);
        addCombo(`2串1:A+B`, [A,B], (pA+pB)/2, oddA*oddB); addCombo(`2串1:A+C`, [A,C], (pA+pC)/2, oddA*oddC);
        addCombo(`2串1:B+C`, [B,C], (pB+pC)/2, oddB*oddC); addCombo(`2串1:B+D`, [B,D], (pB+pD)/2, oddB*oddD);
        ...
        if (totalKellyAmount < budget && totalKellyAmount > 0) {
            ...
            if (activeCombos.length > 0) {
                ...
                for (let c of combos) { if (c.amount > 0 && totalP > 0) { const extra = Math.round(diff * (c.p / totalP)); c.amount += extra; } }
            }
        }
    }
    if (combos.filter(c => c.amount > 0).length === 0) {
        const [A, B, C, D] = matches;
        combos = [ { name: `单关:${A.home}`, fields: [A], amount: 25 }, { name: `单关:${B.home}`, fields: [B], amount: 25 }, { name: `单关:${C.home}`, fields: [C], amount: 25 }, { name: `单关:${D.home}`, fields: [D], amount: 25 } ];
    }
...
function updateStatistics(finishedMatches) {
    ...
    for (const match of finishedMatches) {
        ...
        const cached = optimalCache.get(matchId); const pred = cached.pred; const odds = cached.odds; const weight = cached.weight || 1.0;
        const [h, a] = actualScore.split(':').map(Number); const actualSPF = h > a ? '主队胜' : (h < a ? '客队胜' : '平局'); const totalActual = h + a;
        ...
        for (const [method, data] of Object.entries(methodsToCheck)) {
            ...
            if (data.correct) { stats[method].correct += weight; stats[method].profit += stake * (data.odds - 1); }
            else { stats[method].profit -= stake; }
            stats[method].stake += stake;
        }
    }
    const rates = {};
    for (const method of methods) {
        const total = stats[method].total || 1; const rate = stats[method].correct / total; rates[method] = rate;
        if (rate > bestRate) { bestRate = rate; bestMethod = method; }
        global.historicalMethodSuccess[method] = { hits: stats[method].correct, total: stats[method].total, profit: stats[method].profit, totalStake: stats[method].stake };
    }
...
async function runUpcomingSimulation() {
    ...
    const matchPromises = upcomingSlice.map(async (match) => {
        ...
        try {
            ...
            if (externalSignal) { if (predDeepSeek.half_full && predDeepSeek.half_full.prediction === '胜胜' && externalSignal.includes('主队胜')) { confidenceMultiplier *= 1.1; } }
            if (isCriticalInjury) { if (finalPredJson.half_full && finalPredJson.half_full.prediction === '胜胜') { confidenceMultiplier *= 0.5; } }
            ...
            const odds = await getOdds(home, away, false); cacheEntry.odds = odds;
            let finalPred = {};
            const targetMethod = historicalBestMethod; const bestPred = cacheEntry.best ? cacheEntry.best.pred : null;
            if (bestPred) {
                if (targetMethod === '半全场') finalPred = { method: '半全场', prediction: bestPred.half_full.prediction, analysis: bestPred.half_full.analysis };
                else if (targetMethod === '正确比分') finalPred = { method: '正确比分', prediction: bestPred.correct_score.prediction, analysis: bestPred.correct_score.analysis };
                else if (targetMethod === '让球胜平负') finalPred = { method: '让球胜平负', prediction: bestPred.handicap.prediction, analysis: bestPred.handicap.analysis };
                else if (targetMethod === '总进球数') finalPred = { method: '总进球数', prediction: bestPred.total_goals.prediction, analysis: bestPred.total_goals.analysis };
                else finalPred = { method: targetMethod, prediction: bestPred.win_draw_lose.prediction };
            } else { finalPred = { method: '胜平负', prediction: '平局', analysis: 'AI调用失败，系统启用平局兜底' }; }
            const autoWarnings = [];
...
app.get('/api/state', (req, res) => {
    const matchData = lastMatchData; const finished = matchData.finished || []; const upcoming = matchData.upcoming || [];
    ...
    const safeBet = (bets) => { if (bets && bets.length > 0) return bets; return [ { name: "等待推演中...", fields: [], amount: 0 } ]; };
    let safeBetSummary = { totalProfit: 0, methodProfits: {}, dailyProfits: {}, betRecords: [] };
    ...
    const state = {
        totalAttempts: totalAttempts, finished: finishedWithPred, upcoming: upcomingWithPred,
        bestMethod: global.bestMethod || '胜平负', stats: global.methodStats || {}, rates: global.rates || {},
        externalPredictions: global.externalPredictions || { aiNames: [], predictions: [] },
        upcomingBetsOverall: safeBet(global.upcomingBetsOverall), upcomingBetsWinDrawLose: safeBet(global.upcomingBetsWinDrawLose),
        upcomingBetsHandicap: safeBet(global.upcomingBetsHandicap), upcomingBetsTotalGoals: safeBet(global.upcomingBetsTotalGoals),
        upcomingBetsHF: safeBet(global.upcomingBetsHF), upcomingBetsCS: safeBet(global.upcomingBetsCS), betSummary: safeBetSummary
    };
    res.json(state);
});
